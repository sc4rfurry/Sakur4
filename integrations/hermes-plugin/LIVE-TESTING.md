...
```console
$ curl -s -X POST http://100.98.158.87:8080/v1/chat/completions \
       -H 'Authorization: Bearer local-only' -d '{...}'      → HTTP 200
$ curl -s -X POST http://100.98.158.87:8080/v1/chat/completions -d '{...}'  → HTTP 200
```
...
sakur4d --db ~/.sakur4/hermes.db --backend http://100.98.158.87:8080 \
